...
.. table:: Interrupt interface signals
  ...
  |                         |           | CV32E41P. Specifically irq_i[15:12],             |
  ...
  |                         |           | are a CV32E41P specific extension to the RISC-V  |
...
The ``irq_i[31:0]`` interrupts are controlled via the ``mstatus``, ``mie`` and ``mip`` CSRs. CV32E41P uses the upper 16 bits of ``mie`` and ``mip`` for custom interrupts (``irq_i[31:16]``),
...
For example, in case the parameter FPU is set to 0, the CV32E41P raises an illegal instruction exception for any RVF instruction. The same concerns for XPULP extensions everytime the parameter PULP_XPULP is set to 0 (see :ref:core-integration).
